...
/*
  @api:       POST /api/ai/getInsight
  @desc:      Get AI powered insight
  @access:    Premium
*/
const getInsight = async (req, res, next) => {
  ...
  if (insight) {
    const contentType = await getFetchType(queryType, insight.date);

    if (contentType === "db_content") {
      return res.status(200).json({ status: "success", content: insight.text });
    }
  }

  const promptInitText = getPromptInitText(queryType, language, tradingCode);
  ...
  const userContent = promptInitText + JSON.stringify(dataToFeed);
...
const isUpdatedContentAvailable = (contentDate) => {
  return (
    new Date(contentDate).getTime() === new Date(getAdjustedDate()).getTime()
  );
};

const getFetchType = async (queryType, dbContentDate) => {
  if (queryType === "fairValue") {
    const {
      dataInsertionEnable,
      openHour,
      openMinute,
    ...
    const { isOpen } = marketStatusHelper(
      dataInsertionEnable,
      openHour,
      openMinute,
      closeHour,
  ...
  return isUpdatedContentAvailable(dbContentDate) ? "db_content" : "gpt_api";
};
...
const getPromptInitText = (queryType, language, tradingCode) => {
  let initialText = "";
